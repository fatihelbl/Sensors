...
void TripleSensor ::resetTimer()
{

    startMillis = millis();
}

void TripleSensor::checkTimer()
{
    unsigned long CurrentTime = millis();
    unsigned long ElapsedTime = (CurrentTime - startMillis) / 1000;
    if (ElapsedTime > standByTime)
        waitTime = true;
    else
        waitTime = false;
}